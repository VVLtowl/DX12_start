...
{
	m_EnvConstant.LightColor = XMFLOAT4(3,3,3,1);
	m_EnvConstant.LightDirection = XMFLOAT4(1, 1, -1, 1);

	m_SubsetConstant.Material.Metallic = 1.0f;
	m_SubsetConstant.Material.Specular = 0.5f;
	m_SubsetConstant.Material.Roughness = 0.5f;
...
*/
	...
	//texture load
	m_EnvMapTexture = LoadTexture("Asset\\snow_field_sky_2k_EXR_BC6H_1.dds");
}
...
/*
	...
	FLOAT clearColor[4] = { 0.0f, 4.0f, 0.0f, 1.0f };
	...
	*/
	...
	ImGui_ImplWin32_NewFrame(
		(float)(rect.right - rect.left) / m_BackBufferWidth,
...
void RenderManager::DrawEnd()
{
	...
	//lightings pass
	{
		...
		ImGui::Begin("Material");
		{
			ImGui::SliderFloat(
				"Metallic",
				&m_SubsetConstant.Material.Metallic,
				0.0f,
				1.0f,
				"%.3f");
			ImGui::SliderFloat(
				"Specular",
				&m_SubsetConstant.Material.Specular,
				0.0f,
				1.0f,
				"%.3f");
			ImGui::SliderFloat(
				"Roughness",
				&m_SubsetConstant.Material.Roughness,
				0.0f,
				1.0f,
				"%.3f");

			m_SubsetConstant.Material.BaseColor = XMFLOAT4(1, 0, 0, 1);
			m_SubsetConstant.Material.EmissionColor = XMFLOAT4(0, 1, 0, 1);

			//m_SubsetConstant.Color = XMFLOAT4(1, 0, 1, 1);

			SetConstant(RenderManager::CONSTANT_TYPE::SUBSET, &m_SubsetConstant, sizeof(m_SubsetConstant));
		}
		ImGui::End();
		...
		ImGui::Begin("Light");
		{
			//set light
			{
				ImGui::SliderFloat(
					"color r",
					&m_EnvConstant.LightColor.x,
					0.0f,
					5.0f,
					"%.3f");
				ImGui::SliderFloat(
					"color g",
					&m_EnvConstant.LightColor.y,
					0.0f,
					5.0f,
					"%.3f");
				ImGui::SliderFloat(
					"color b",
					&m_EnvConstant.LightColor.z,
					0.0f,
					5.0f,
					"%.3f");
				...
				ImGui::SliderFloat(
					"dir x",
					&m_EnvConstant.LightDirection.x,
					-1.0f,
					1.0f,
					"%.3f");
				ImGui::SliderFloat(
					"dir y",
					&m_EnvConstant.LightDirection.y,
					-1.0f,
					1.0f,
					"%.3f");
				ImGui::SliderFloat(
					"dir z",
					&m_EnvConstant.LightDirection.z,
					-1.0f,
					1.0f,
					"%.3f");

				SetConstant(RenderManager::CONSTANT_TYPE::ENV, &m_EnvConstant, sizeof(m_EnvConstant));
			}
		}
		ImGui::End();
	...
	//post effect
	{
		...
		m_GraphicsCommandList->OMSetRenderTargets(
			1,
			&m_RenderTargetHandle[m_RTIndex],
			TRUE,
			&m_DepthBufferHandle);
	...
	//imgui
	{
		...
		ImGui::Begin("G buffer");
		{
			...
			ImGui::Image((void*)m_ColorBuffer->SRVHandle.ptr, ImVec2(250, 125));


			ImGui::Text("normal buffer");
			ImGui::Image((void*)m_NormalBuffer->SRVHandle.ptr, ImVec2(250, 125));


			ImGui::Text("position buffer");
			ImGui::Image((void*)m_PositionBuffer->SRVHandle.ptr, ImVec2(250, 125));


			ImGui::Text("arm buffer");
			ImGui::Image((void*)m_ArmBuffer->SRVHandle.ptr, ImVec2(250, 125));


			ImGui::Text("emission buffer");
			ImGui::Image((void*)m_EmissionBuffer->SRVHandle.ptr, ImVec2(250, 125));
		}
		ImGui::End();
...
void RenderManager::SetConstant(CONSTANT_TYPE Type, const void* Constant, unsigned int Size)
{
	...
	memcpy(
		m_ConstantBufferPointer[m_RTIndex] + CONSTANT_BUFFER_SIZE * m_ConstantBufferIndex[m_RTIndex], 
		Constant,
		Size);
